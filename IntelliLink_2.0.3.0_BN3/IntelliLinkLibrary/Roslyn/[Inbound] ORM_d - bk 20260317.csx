...
public Dictionary<string, string> SourceToDestination(Dictionary<string, string> mapper, string message)
{
    try
    {
        ...
        var encodedHl7 = xdoc.Element("message")?.Value;
        if (string.IsNullOrEmpty(encodedHl7))
        {
            var errMsg = "Encoded HL7 message is null or empty.";
            Log.GetLogger(_linkerName).Error(errMsg);
            throw new Exception(errMsg);
        }

        var imageBytes = Convert.FromBase64String(encodedHl7);
        var decodedText = Encoding.UTF8.GetString(imageBytes);

        Log.GetLogger(_linkerName).Info($"Decoded HL7:{Environment.NewLine}" + decodedText);
        ...
        // XmlMapper를 사용하여 Dictionary로 변환 및 기존 mapper와 병합
